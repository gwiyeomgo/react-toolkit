 ...
 */
export const checkTimeDifferenceWithinRange = (
  targetTime: Date,
  limitHours: number,
) => {
  ...
  const timeDifferenceHours = (targetTime.getTime() - now.getTime()) / 3600000;
 ...
 */
export const checkTimePassed = (targetTime: Date) => {
  const currentTime = new Date();
  return targetTime.getTime() - currentTime.getTime() <= 0;
};
/**
 ...
 * isNaN(false)       // false
 **/
export const ConvertVerifyStringToNumber = (str: string) => {
  return !isNaN(Number(str)) && !isNaN(parseFloat(str)); //true && true
};
